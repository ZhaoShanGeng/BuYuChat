...
use aisdk::core::{LanguageModelStreamChunkType, language_model::StopReason};
...
async fn run_non_stream_generation(
    ...
    finalize_completion(state, request, completion, None).await
}
...
async fn run_stream_generation(
    ...
    loop {
        tokio::select! {
            ...
            maybe_chunk = handle.response.stream.next() => {
                match maybe_chunk {
    ...
    let completion = AiChatCompletion {
        ...
        finish_reason: map_stream_stop_reason(handle.response.stop_reason().await),
        model: request.config.model_name.clone().unwrap_or_default(),
    };
...
async fn finalize_completion(
    ...
    if existing_content_bytes.unwrap_or(0) == 0 {
...
/// 把 AISDK 的 stop reason 映射为对外暴露的 finish reason。
fn map_stream_stop_reason(stop_reason: Option<StopReason>) -> String {
    match stop_reason {
